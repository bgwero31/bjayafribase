...
export default function Marketplace() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      get(phoneRef).then((snap) => {
        ...
        console.warn("Could not read whatsapp number:", err);
      });
    }
  }, []);
  ...
  const deleteComment = (productId, commentId) => {
    remove(ref(db, `products/${productId}/comments/${commentId}`)).catch((err) =>
  ...
  const toggleLike = (p) => {
    ...
    const liked = Array.isArray(p.likes) && p.likes.includes(uid);
    const newLikes = liked ? p.likes.filter((id) => id !== uid) : [...(p.likes || []), uid];
    update(ref(db, `products/${p.id}`), { likes: newLikes }).catch((err) =>
  ...
  const toggleDislike = (p) => {
    const uid = auth.currentUser?.uid;
    if (!uid) return alert("Login first");
    const dis = Array.isArray(p.dislikes) && p.dislikes.includes(uid);
    const newDislikes = dis ? p.dislikes.filter((id) => id !== uid) : [...(p.dislikes || []), uid];
    update(ref(db, `products/${p.id}`), { dislikes: newDislikes }).catch((err) =>
